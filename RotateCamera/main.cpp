...
int main(void)
{
	...
	// Create window
	// -------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Rotate Camera", NULL, NULL);
	if (window == NULL)
	{
	...
	// --------------------------------
	Shader myShader = Shader("../RotateCamera/shader.vert", "../RotateCamera/shader.frag");
	// --------------------------------
	...
	light.position = glm::vec3(0.0f, 0.0f, 1.5f);
